...
const Discovery = () => {
    const {offline} = useSelector((i:RootState)=>i.AuthSlice)

  return (
    <View style={{flex:1,backgroundColor:"white"}}>
      <TopBar title={"Offline Saved"} hideBookmark hideDownload/>
      <View style={{padding:20}}>
            <FlatList
                showsVerticalScrollIndicator={false}
                data={offline}
                renderItem={({item,index})=><ProductsListLayoutSingle item={item}/>}
